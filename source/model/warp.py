...
class BackwardWarp(nn.Module):
    ...
    def _interpolate(self, img, x, y, out_height, out_width):
        ...
        x1_f = x0_f + 1.0
        y1_f = y0_f + 1.0
        x0 = x0_f.clone().detach().long().to(img.device)
        y0 = y0_f.clone().detach().long().to(img.device)

        x1 = torch.clamp(x1_f, 0, width_f-1).clone().detach().long().to(img.device)
        y1 = torch.clamp(y1_f, 0, height_f-1).clone().detach().long().to(img.device)

        dim1 = width * height
        dim2 = width
        base = (torch.arange(num_batch) * dim1).clone().detach().long().to(img.device)
